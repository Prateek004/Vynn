...
export async function syncMenuToSupabase(userId: string): Promise<void> {
  ...
  try {
    const { data: { user } } = await sb.auth.getUser();
    if (!user) return;
    const [cats, items] = await Promise.all([dbGetAllCategories(userId), dbGetAllMenuItems(userId)]);
    if (cats.length > 0) {
      await sb.from("menu_categories").upsert(
        cats.map((c) => ({ id: c.id, user_id: user.id, name: c.name, sort_order: c.sortOrder })),
        { onConflict: "id" }
      );
    }
    if (items.length > 0) {
      await sb.from("menu_items").upsert(
        items.map((i) => ({
          id: i.id, user_id: user.id, category_id: i.categoryId,
          name: i.name, price_paise: i.pricePaise, is_available: i.isAvailable,
          has_variants: !!(i.portionEnabled && i.portions?.length),
          variants: i.portions ?? [], addons: i.addOns ?? [],
...
// FIX: uid required — prevents syncing wrong user's orders
