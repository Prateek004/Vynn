...
export default function SettingsPage() {
  ...
  const showOpenTableToggle = OPEN_TABLE_BIZ_TYPES.includes(
  ...
  const [openTableBilling, setOpenTableBilling] = useState(
    ss.openTableBilling ?? false
  );
  const [saving, setSaving] = useState(false);

  // Keep form in sync when session changes (e.g. after login)
  useEffect(() => {
    if (!session) return;
  ...
  const handleSave = async () => {
    ...
    // setSession persists to localStorage AND updates React state
    setSession(updated);

    if (isSupabaseEnabled()) {
      try {
        const sb = await import("@/lib/supabase/client").then((m) =>
          m.getSupabase()
        );
        if (sb) {
          const {
  ...
  const handleReset = () => {
    if (
      !confirm(
        "Reset ALL local data? This will clear your orders, menu, and session. Cannot be undone."
      )
    )
      return;
    try {
    ...
    try {
      // Delete vynn_db (current) and servezy_db (legacy) if they exist
      indexedDB.deleteDatabase("vynn_db");
      indexedDB.deleteDatabase("servezy_db");
    } catch {}
      ...
      <div className="flex flex-col min-h-screen bg-gray-50">
        <div className="bg-white px-4 pt-12 lg:pt-5 pb-4 shadow-sm">
          ...
          <p className="text-xs text-gray-400 mt-0.5">
            Changes save to your device instantly
          </p>
        </div>
          ...
          <Section title="Billing">
            <div className="px-4 py-3">
              ...
              <p className="text-xs text-gray-400 mt-1">
                Used to generate QR code at checkout
              </p>
            </div>
          </Section>
              ...
              <div className="px-4 py-3 space-y-4">
                    ...
                    <div className="flex gap-2 flex-wrap">
                      {[5, 10, 15, 20, 25, 30].map((n) => (
                        <button
                          ...
                          className={`w-10 h-9 rounded-xl border-2 text-sm font-bold press transition-all ${
                            tableCount === n
                              ? "border-primary-500 bg-primary-50 text-primary-600"
                              : "border-gray-200 text-gray-600"
                          }`}
                  ...
                  <>
                    ...
                    {openTableBilling && (
                      <p className="text-xs text-amber-600 bg-amber-50 rounded-xl px-3 py-2">
                        Active — go to Orders → Open Tables to manage running
                        tabs.
                      </p>
                    )}
                  </>
                ...
                {showBarToggle && (
                  <>
                    ...
                    <Toggle
                      label="Bar Tab"
                      desc="Enable bar section in Stock management"
                      value={barEnabled}
                      onChange={setBarEnabled}
                    />
                    {barEnabled && (
                      <p className="text-xs text-amber-600 bg-amber-50 rounded-xl px-3 py-2">
                        Bar tab is active. Go to Stock → Bar to manage bar
                        inventory.
                      </p>
                    )}
                  </>
          ...
          <Section title="Danger Zone">
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs text-gray-400">
                  Clears orders, menu, and session
                </p>
              </div>
            </button>
          </Section>
